...
bool parking(float &cmd_fligh_x,float &cmd_flight_y,float height, uint8_t detection_flag=0,DJIDrone* drone);
bool cbarrier(float &cmd_fligh_x,float &cmd_flight_y, float &cmd_yaw, float height,float set_height, uint8_t detection_flag=0,DJIDrone* drone);
	...
	switch ( drone->transparentdata.data.at ( 0 ) )
	{
	  ...
	  case 'd':  
	  {
	    ...
	    state_in_mission_pub.publish(state_msg);
		 ...
		 flip_once= cbarrier(filtered_x,filtered_y,filtered_yaw,1.2,2.0,Detection_fg,drone);
		 if(flip_once)
		 {
		   state_in_mission=2;
		   flip_once=false;
		 ...
		 flip_once= cbarrier(filtered_x,filtered_y,filtered_yaw,1.4,2.2,Detection_fg,drone);
		 if(flip_once)
		 {
		   state_in_mission=4;
		   flip_once=false;
		 ...
		 flip_once= cbarrier(filtered_x,filtered_y,filtered_yaw,1.6,2.4,Detection_fg,drone);
		 if(flip_once)
		 {
		   state_in_mission=5;
		   flip_once=false;
		 ...
		 flip_once= cbarrier(filtered_x,filtered_y,filtered_yaw,1.8,2.6,Detection_fg,drone);
		 if(flip_once)
		 {
		   state_in_mission=8;
		   flip_once=false;
...
bool parking(float &cmd_fligh_x,float &cmd_flight_y,float height, uint8_t detection_flag,DJIDrone* drone)
{
  ...
  if(land_mode==0)   //closing to landing part and landing
  {
    if(detection_flag)  //parking pad detected
    {
      if(abs(cmd_fligh_x)>50||abs(cmd_flight_y)>50)  //closing and keep height
      {
	...
	drone->attitude_control(0x9B,cmd_fligh_x,cmd_flight_y,flying_height_control_tracking,0);
      ...
      else if(abs(cmd_fligh_x)<50&&abs(cmd_flight_y)<50) 
      {
	if(height>1.5)
	  drone->attitude_control(0x9B,cmd_fligh_x,cmd_flight_y,1.5,0);   //TODO: height adjusting
	  else
	  {
	    drone->landing();
	    land_mode=1;
  ...
  else if(land_mode==1)   //automatic takeoff
  {
    sleep(5);
    drone->takeoff();
    if(ob_distance[0]>1.0)   //TODO: adjusting using the land pad.
    {
      land_mode=0;
      return true;
    }
...
bool cbarrier(float &cmd_fligh_x,float &cmd_flight_y, float &cmd_yaw,float height, float set_height, uint8_t detection_flag=0,DJIDrone* drone)
{
  ...
  else if(cbring_mode==1)  //closing to the hanging pad. first x,y then z
  {
    ...
    drone->attitude_control ( 0x9B,cmd_fligh_x-0.5,cmd_flight_y,flying_height_control_tracking,cmd_yaw);
    if(abs(cmd_fligh_x-0.5)<0.3&&abs(cmd_flight_y)<0.3&&abs(cmd_yaw)<10)
      cbring_mode=2;
  }
